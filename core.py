...
try:
    from langfuse import Langfuse as _LangfuseClient  # type: ignore
except Exception:
        ...
        *,
        ...
        if self.provider == "mlflow":
            if not hasattr(mlflow, "genai"):
                raise ImportError(
                    "mlflow.genai is not available. Please upgrade mlflow to a version that supports genai."
                )
            kwargs: Dict[str, Any] = {
                "name": name,
                "template": template,
        ...
        if self.provider == "langfuse":
            derived_type = prompt_type or (
                "chat" if _is_chat_template(template) else "text"
            )
            ...
            if derived_type == "chat" and not _is_chat_template(template):
                raise ValueError(
                    "For prompt_type='chat', template must be a list of {role, content} dicts"
                )
        ...
        *,
        ...
        if self.provider == "mlflow":
            ...
            if target is None:
                if not name or version is None:
                    raise ValueError(
                        "For mlflow, provide either 'path' or both 'name' and 'version'"
                    )
                if not isinstance(version, int) or version < 1:
                    raise ValueError("version must be a positive integer")
                target = f"prompts:/{name}/{version}"
            if not isinstance(target, str) or not re.match(
                r"^prompts:/[^/]+/\d+$", target
            ):
                raise ValueError(
                    "mlflow 'path' must be of the form 'prompts:/<name>/<version>'"
                )
            try:
                prompt_obj = mlflow.genai.load_prompt(target)  # type: ignore[attr-defined]
            except Exception as e:
                raise PromptFetchError(
                    f"MLflow load_prompt failed for '{target}': {e}"
                ) from e
            ...
            return PromptHandle(
                self.provider, prompt_obj, name=name_extracted, version=ver_extracted
            )
        ...
        if self.provider == "langfuse":
            ...
            if not name or not isinstance(name, str):
                raise ValueError(
                    "For langfuse, 'name' is required and must be a string"
                )
...
def _validate_template(template: Any) -> None:
    ...
    if _is_chat_template(template):
        return
    raise ValueError(
        "template must be a string or a chat template list of {role, content} dicts"
    )
    ...
    def __init__(
        ...
        *,
        ...
        self.version = (
            ...
            else getattr(self.underlying, "version", None)
        )
    ...
    def compile(self, **variables: Any) -> Any:
        try:
            ...
            raise PromptCompilationError(
                f"Failed to compile prompt for provider '{self.provider}': {e}"
            ) from e
        raise PromptProviderError("Unsupported provider; use 'mlflow' or 'langfuse'")
...
def _ensure_mlflow(
    ...
    if load_env:
        try:
            ...
            pass
    # Prefer CENTRALPROMPT_* overrides if present
    uri = (
        tracking_uri
        or os.getenv("AILEVATE_PROMPT_MLFLOW_TRACKING_URI")
    ...
    exp = (
        experiment
        or os.getenv("AILEVATE_PROMPT_MLFLOW_EXPERIMENT_NAME")
...
def _ensure_langfuse(load_env: bool = True):
    ...
    if load_env:
        try:
            ...
            pass
    # Simple approach: try langfuse.init with env-provided creds; otherwise try get_client with kwargs.
    public_key = os.getenv("AILEVATE_PROMPT_LANGFUSE_PUBLIC_KEY")
    secret_key = os.getenv("AILEVATE_PROMPT_LANGFUSE_SECRET_KEY")
    base_url = os.getenv("AILEVATE_PROMPT_LANGFUSE_BASE_URL") or os.getenv(
        "AILEVATE_PROMPT_LANGFUSE_HOST"
    )

    # Require both public_key and secret_key for a fully functional client
    if not public_key:
        raise ImportError(
            "Langfuse public_key not configured. Set AILEVATE_PROMPT_LANGFUSE_PUBLIC_KEY in your environment/.env."
        )
    if not secret_key:
        raise ImportError(
            "Langfuse secret_key not configured. Set AILEVATE_PROMPT_LANGFUSE_SECRET_KEY in your environment/.env."
        )
    try:
        if base_url is not None:
            try:
                ...
                # Some versions expect 'host' instead of 'base_url'
                return _LangfuseClient(
                    public_key=public_key, secret_key=secret_key, host=base_url
                )
