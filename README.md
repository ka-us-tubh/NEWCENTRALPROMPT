...
# Prefer AILEVATE_PROMPT_MLFLOW_TRACKING_URI (fallback to MLFLOW_TRACKING_URI). Optionally AILEVATE_PROMPT_MLFLOW_EXPERIMENT_NAME or MLFLOW_EXPERIMENT_NAME.
...
- **experiment** (MLflow): Optional experiment name. Tracking URI is taken from `.env`/ENV via `AILEVATE_PROMPT_MLFLOW_TRACKING_URI` or `MLFLOW_TRACKING_URI`.
...
- MLflow: `.env` or environment variables (precedence: AILEVATE_PROMPT_* overrides standard vars)
  - `AILEVATE_PROMPT_MLFLOW_TRACKING_URI` or `MLFLOW_TRACKING_URI`
  - `AILEVATE_PROMPT_MLFLOW_EXPERIMENT_NAME` or `MLFLOW_EXPERIMENT_NAME` (optional if passed in code)
- Langfuse: `.env` or environment variables (precedence: AILEVATE_PROMPT_* overrides standard vars)
  - `AILEVATE_PROMPT_LANGFUSE_PUBLIC_KEY` or `LANGFUSE_PUBLIC_KEY`
  - `AILEVATE_PROMPT_LANGFUSE_SECRET_KEY` or `LANGFUSE_SECRET_KEY`
  - `AILEVATE_PROMPT_LANGFUSE_BASE_URL` (or `AILEVATE_PROMPT_LANGFUSE_HOST`) or `LANGFUSE_BASE_URL` (or `LANGFUSE_HOST`)
  - The library passes these values directly to the Langfuse client when constructing it.
